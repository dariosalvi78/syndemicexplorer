import { Pool } from '../db.js';
    ...
    try {
      ...
      if(!countryCode && !indicator && !area1Code && !level) {
        indicator = 'confirmed';
        query = getQuery(indicator, ' ');
        data = await Pool.query(query);
      } else if (!countryCode || !indicator || !area1Code || !level) {
          res.sendStatus(400);
          return;
      }
      else {
        query = getQuery(indicator, 'where epidemiology.country_code = $1 and epidemiology.area1_code = $2 and admin_areas.level = $3')
        data = await Pool.query(query, [countryCode, area1Code, level])
      }
      let features = [];
